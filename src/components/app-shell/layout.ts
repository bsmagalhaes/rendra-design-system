/*
 * LAYOUT DO APPSHELL
 * Tipo do layout e padrão interno do componente. O projeto escolhe os valores no
 * briefing e passa como prop `layout` (Partial<ShellLayout>) ao <AppShell>; sem a prop
 * (ou faltando alguma chave), vale o padrão abaixo. src/config/layout.ts reexporta este
 * módulo para o boilerplate e para as predefinições de src/config/presets.ts.
